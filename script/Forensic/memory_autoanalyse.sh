...
DMP_dir=/home/sansforensics/Desktop/cases/
  ...
  if [ -d $dumpmem.output ]; then 
    ...
    if [ -f $dumpmem.profile ] && [ -s $dumpmem.profile ]; then 
      ...
      then
        ...
        curl -X POST "127.0.0.1:5601/api/saved_objects/_import?overwrite=true" -H "kbn-xsrf: true" --form file=@/home/sansforensics/Desktop/DFIR-tools/kibana/kibana--dashboard-volatility-processes-investigation.ndjson      
